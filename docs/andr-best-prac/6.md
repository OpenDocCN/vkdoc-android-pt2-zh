...
你可以看到 Android 领域的恶意软件应用数量确实在急剧增长。该报告继续说，在 15k 个应用中，发现的恶意软件特征列于[表 6-1](#Tab1) 。
...
*   **Apply secure coding techniques.** There shouldn’t be any need to open a file as `WORLD_READABLE` or `WORLD_WRITEABLE` as done in [Listing 6-1](#list1); the default behavior is not to open a file as `WORLD_READABLE` or `WORLD_WRITEABLE` See.
    ...
    [清单 6-1](#_list1) 。不安全的技术-打开一个文件作为世界可读，世界可写 T3】
...
[清单 6-11](#list11) 显示了呼叫中心应用第一版的 Settings.java 文件的源代码。这个版本包括一些非常明显的违反我们在本章介绍的安全最佳实践的地方。在继续下面的 SPE 输出之前，花些时间浏览一下代码，看看是否能发现这些问题。
