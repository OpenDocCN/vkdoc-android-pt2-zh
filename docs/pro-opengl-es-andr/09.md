...
尽可能多地批处理依赖于相同状态的操作，因为改变系统状态(通过使用`glEnable()`和`glDisable()`调用)代价很高。OpenGL 不会在内部检查某个特定的特性是否已经处于您想要的状态。在本书的练习中，我会比我可能需要的更频繁地设置状态，以确保行为是容易预测的。但是对于商业的、性能密集型的应用程序，在发布版本中尽量去掉多余的调用。
