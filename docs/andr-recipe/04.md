...
为了在您的应用程序中使用 Maps API，必须首先从 Google 获得 API 密钥。此密钥是使用签名应用程序的私钥生成的。如果没有 API 键，可以使用映射类，但不会向应用程序返回地图切片。
...
如果您无法直接检索要在应用程序中显示的内容的位置，该应用程序必须在将 URL 传递给 MediaPlayer 之前跟踪重定向路径。清单 4–27 是一个简单的 AsyncTask 跟踪程序的例子。
