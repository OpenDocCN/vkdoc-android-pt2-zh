...
在[表 8-1](#Tab1) 中列出的命令有助于将被调试应用从命令行转移到目标设备或仿真上。这非常有用，尤其是在没有 ssh 终端连接的情况下。
...
为 VirtualBox 设置启动参数非常简单。进入关于此手机的设置![image](img/arrow.jpg)，查看图 8-17 中[所示的制造信息。](#Fig17)
...
最后，SVEN Debug console 是一个命令行实用程序，它连接到 Nexus 并观察由 SVEN-TX 检测代码(驱动程序、用户应用和库)生成的所有事件。可脚本化的过滤器动态地接受或拒绝任何可描述的事件类别(例如，仅记录来自 MPEG 解码器的事件)。可编写脚本的“触发器”停止记录事件，以暂停导致故障的事件的本地捕获。反向工程功能将所有寄存器读/写从物理地址转移到单元和外部架构规范(EAS)寄存器。
