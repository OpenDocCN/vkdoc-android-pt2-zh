...
现在你可以添加 Java **扩展**关键字 T3】和 **Activity** 类，你需要扩展它们来使这个类成为一个 Android Activity，它在很大程度上用于保存用户界面设计。
...
才能调用标准活动**。** **setContentView( )** 方法，您需要创建**布局** XML 目录，并将素材放置在项目的这个**移动**部分中。右击 **/mobile/res** 文件夹，选择**新建![image](img/image00490.jpeg) Android 资源目录**菜单序列。使用**资源类型**下拉框选择**布局**；这也将设置文件夹名称，如图[图 13-12](#Fig12) 所示。
...
DataItem 对象包含数据的“blob”以及相关联的素材。使用包含其创建者和路径的 Uri 来标识 DataItem。一个 DataItem 类有**四个**公共方法:T2。getAssets( ) 方法检索包含在 DataItem 中的素材的**映射**；**。getData( )** 方法检索存储在指定 Uri 的数据的**数组**；**。getUri( )** 方法返回 DataItem**Uri**；以及**。setData(byte[ ] data)** 方法在 DataItem 内部设置一个**数据 byte[ ]数组**。让我们在代码中实现一个数据项！
