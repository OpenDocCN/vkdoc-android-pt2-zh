...
使用**运行![image](img/image00490.jpeg)运行‘ProWatchFaceCompanionConfigActivity’**菜单序列，运行应用程序代码，查看它是否运行并检查 UI。
...
下一次我测试该应用程序时，我注意到 IntelliJ 正在使用一个 Wear Round AVD，这很奇怪，所以我使用了一个**运行![image](img/image00490.jpeg)编辑配置**菜单序列来检查 IntelliJ 当前使用的设置。
