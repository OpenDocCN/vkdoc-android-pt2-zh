...
第一个`setContentView()`告诉应用程序加载`main.xml`作为布局。按钮部分监听按钮的点击。一旦发生这种情况，你调用另一个`setContentView()`在屏幕上显示`GameView`。这是你用来初始化游戏的简单方法。
...
> 4.  The next screen is shown in [Figure 10-6](#fig_10_6) . Fill in with accurate and professional information. If you don't have a website, that's fine, but you might want one. ![images](images/1006.jpg) T12 * T14 】 Figure 10-6\. Create your robot application market account*
