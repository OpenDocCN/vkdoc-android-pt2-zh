...
在我们开始将 AVILib 集成到我们现有的`MoviePlayer`项目之前，我们需要首先向它添加原生支持。右键单击`MoviePlayer`项目并从上下文菜单中选择**Android Tools**![Image](img/U001.jpg)**Add Native Support…**以启动添加原生支持对话框。单击“完成”按钮使用默认参数。塞阔雅(我们在上一章安装的)将修改项目配置，它还将包括一组样板文件，如图[图 8-2](#fig_8_2) 所示。
...
为了确保我们现在可以构建 AVILib，从顶部菜单栏选择**项目** ![Image](img/U001.jpg) **构建项目**来构建`MoviePlayer`项目。如果一切顺利，你会看到“构建完成”的提示信息，如图 *** [图 8-4 ](#fig_8_4) *** 所示。
...
要开始实现 AVI 播放器的 Java 部分，通过项目浏览器选择`com.apress.movieplayer` Java 包，从顶部菜单栏选择**文件** ![Image](img/U001.jpg) **新建** ![Image](img/U001.jpg) **类**，将新类命名为`AviPlayer`。[清单 8-2](#list_8_2) 显示了`AviPlayer`类文件的内容。
...
从顶部菜单栏选择**文件** ![Image](img/U001.jpg) **新建** ![Image](img/U001.jpg) **其他…**![Image](img/U001.jpg)**Android**![Image](img/U001.jpg)**Android XML 布局文件**将一个新的布局文件添加到项目中作为`avi_player.xml`。布局将只包含一个填充整个屏幕的`SurfaceView`对象。[清单 8-13](#list_8_13) 显示了布局文件的内容。
...
从顶部菜单栏选择**文件** ![Image](img/U001.jpg) **新建** ![Image](img/U001.jpg) **类**以启动新建 Java 类对话框。新类将扩展`Activity`类，并实现`SurfaceHolder.Callback`接口，如图[图 8-6](#fig_8_6) 所示。
