...
塞阔雅可以通过 Eclipse 插件存储库获得。启动 Eclipse，从顶部菜单栏选择**帮助** ![Image](img/U001.jpg) **安装新软件……**，启动安装向导。对于“使用”字段，选择 Indigo 存储库。在 Work with 字段下面的过滤文本字段中键入**塞阔雅**，Eclipse 将过滤可用插件的列表。展开移动和设备开发类别，选择塞阔雅 Android 原生代码支持，如图[7-14](#fig_7_14)所示。单击“下一步”按钮继续。
...
塞阔雅需要知道安卓 NDK 的安装位置才能运行。在 Windows 和 Linux 上选择**窗口** ![Image](img/U001.jpg) **首选项**，或者在 Mac OS X 上选择 **Eclipse** ![Image](img/U001.jpg) **首选项**，启动首选项对话框。在首选项对话框中，展开 Android 类别，然后选择原生开发。点击浏览按钮，选择 NDK 位置，如图[图 7-16](#fig_7_16) 所示。
...
从顶部菜单栏选择**文件** ![Image](img/U001.jpg) **新建** ![Image](img/U001.jpg) ** Android 项目**，启动新建 Android 项目向导。将项目命名为`HelloJni`，并选择“从现有源代码创建项目”选项。然后点击浏览按钮，选择`<*NDK Directory*>\samples\hello-jni`作为位置，如图[图 7-17](#fig_7_17) 所示。单击“下一步”按钮继续。
...
虽然示例项目包含本机代码，但 ADT 将无法构建它。您需要首先向项目添加本机代码支持，以允许塞阔雅在 Android 应用程序构建过程中构建本机代码。右键点击项目，在右键菜单中选择 **Android Tools** ![Image](img/U001.jpg) **添加原生支持**，如图[图 7-18](#fig_7_18) 所示。
...
首先，我们需要定义一个变量，该变量允许我们在定义外部工具时指向 Android 框架 JAR 文件。在 Eclipse 中，在 Windows 和 Linux 上选择**窗口** ![Image](img/U001.jpg) **首选项**，或者在 Mac OS X 上选择 **Eclipse** ![Image](img/U001.jpg) **首选项**，打开首选项对话框。要过滤列表，输入**字符串替换**，如图[图 7-21](#fig_7_21) 所示。
...
为了简化`javah`的使用，我们将使用 Eclipse 定义一个新的外部工具。从顶部菜单栏选择**运行** ![Image](img/U001.jpg) **外部工具** ![Image](img/U001.jpg) **外部工具配置…** 。在“外部工具配置”对话框中，选择“程序”，然后单击“新建启动配置”按钮。如下填写工具信息，如[图 7-23](#fig_7_23) 所示:
...
和`javah`一样，我们将使用 Eclipse 为`javap`定义一个新的外部工具。从顶部菜单栏选择**运行** ![Image](img/U001.jpg) **外部工具** ![Image](img/U001.jpg) **外部工具配置**。在外部工具配置对话框中，选择程序并单击新建启动配置按钮。如下填写工具信息，如图[图 7-24](#fig_7_24) 所示:
...
要使用`javap`外部工具，选择一个带有本地方法的类文件，并从顶部菜单栏中选择**运行** ![Image](img/U001.jpg) **外部工具** ![Image](img/U001.jpg) **javap** 。Eclipse 将首先构建项目，以确保类文件是最新的。然后`javap`工具将解析编译后的 Java 类，并将字段和方法签名输出到控制台视图，如图[图 7-25](#fig_7_25) 所示。
...
按照上一节描述的相同步骤，为`ndk-gdb-eclipse`脚本定义一个新的外部工具配置。启动 Eclipse，从顶部菜单栏选择**运行** ![Image](img/U001.jpg) **调试配置**。在“调试配置”对话框中，选择 C/C++应用程序，并单击“新建”图标来定义新的调试配置。如[图 7-28](#fig_7_28) 所示，填写工具信息如下:
...
3.  启动 Java 调试会话。
4.  当调试器到达断点时，选择项目，然后通过选择**运行**![Image](img/U001.jpg)T5】外部工具 ![Image](img/U001.jpg) **ndk-gdb-eclipse** 启动您之前定义的`ndk-gdb-eclipse`外部工具。
...
现在你可以使用`ndk-stack`工具了。一个应用崩溃后，选择**运行** ![Image](img/U001.jpg) **外部工具** ![Image](img/U001.jpg) **ndk-stack** 。工具将被执行，输出将显示在控制台视图中，如图[图 7-34](#fig_7_34) 所示。
