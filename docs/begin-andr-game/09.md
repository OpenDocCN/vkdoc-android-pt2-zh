...
![9781430246770_Fig09-01.jpg](images/9781430246770_Fig09-01.jpg)
...
![9781430246770_Fig09-02.jpg](images/9781430246770_Fig09-02.jpg)

[图 9-2。](#_Fig2)超级跳线的所有屏幕和过渡
...
![9781430246770_Fig09-03.jpg](images/9781430246770_Fig09-03.jpg)
...
![9781430246770_Fig09-04.jpg](images/9781430246770_Fig09-04.jpg)
...
你可能想知道为什么没有指向右边的箭头。记住第八章中的内容，使用我们的高级精灵批处理器，我们可以通过指定负的宽度和/或高度值来轻松翻转我们绘制的东西。我们将对几个图形资产使用这个技巧来节省一些内存。

接下来是我们在主菜单屏幕上需要的元素:徽标、菜单项和背景图像。图 9-5 显示了所有这些元素。

![9781430246770_Fig09-05.jpg](images/9781430246770_Fig09-05.jpg)
...
![9781430246770_Fig09-06.jpg](images/9781430246770_Fig09-06.jpg)
...
![9781430246770_Fig09-07.jpg](images/9781430246770_Fig09-07.jpg)
...
![9781430246770_Fig09-08.jpg](images/9781430246770_Fig09-08.jpg)
...
![9781430246770_Fig09-09.jpg](images/9781430246770_Fig09-09.jpg)
...
![9781430246770_Fig09-10.jpg](images/9781430246770_Fig09-10.jpg)
...
![9781430246770_Fig09-11.jpg](images/9781430246770_Fig09-11.jpg)

[图 9-11。](#_Fig11)硬币及其动画帧

[图 9-11](#Fig11) 中的图像为 96×32 像素，每帧为 32×32 像素。

关于图 9-12 中的弹簧图像，不必多说。春天只是快乐地坐在图像的中心。该图像为 32×32 像素。

![9781430246770_Fig09-12.jpg](images/9781430246770_Fig09-12.jpg)

[图 9-12。](#_Fig12)春天

图 9-13 中的城堡也不是动画。它比其他对象大(64×64 像素)。

![9781430246770_Fig09-13.jpg](images/9781430246770_Fig09-13.jpg)

[图 9-13。](#_Fig13)城堡

[图 9-14](#Fig14) (64x64 像素)中的平台有四个动画帧。根据我们的游戏机制，一些平台在鲍勃击中时会被粉碎。我们将播放一次这种情况下平台的完整动画。对于静态平台，我们只使用第一个框架。

![9781430246770_Fig09-14.jpg](images/9781430246770_Fig09-14.jpg)
...
我们是否也为所有其他图像创建单独的纹理？不。我们创建一个单一的纹理图谱。事实证明，其他所有东西都可以很好地放在一个 512×512 像素的地图集里，我们可以将它作为一个单一的纹理来加载——这将使 GPU 非常高兴，因为我们只需要为所有游戏元素绑定一个纹理，除了背景和帮助屏幕图像。[图 9-15](#Fig15) 所示为图集。

![9781430246770_Fig09-15.jpg](images/9781430246770_Fig09-15.jpg)
...
![9781430246770_Fig09-16.jpg](images/9781430246770_Fig09-16.jpg)
...
接下来，我们有一个神秘的方法叫做 reload()。请记住，当我们的应用程序暂停时，OpenGL ES 上下文将会丢失。当应用程序恢复时，我们必须重新加载纹理，这正是这个方法要做的。如果启用了声音，我们还会恢复音乐播放。
...
让我们从 Spring 类开始，它出现在[清单 9-9](#list9) 中。
...
World 类中的最后一个方法在 World.update()方法的最后一行被调用，如清单 9-19 中的[所示。](#list19)

[**清单 9-19。**World.java 其余地区的](#_list19)；游戏结束检查方法
...
我们即将完成超级 Jumper。我们需要实现的最后一件事是游戏屏幕，它将把实际的游戏世界呈现给玩家，并允许玩家与之进行交互。游戏屏幕由五个子屏幕组成，如本章前面的[图 9-2](#Fig2) 所示。我们有就绪屏幕、正常运行屏幕、下一级屏幕、游戏结束屏幕和暂停屏幕。《诺姆先生》中的游戏画面与此类似；它只缺少一个下一级屏幕，因为只有一个级别。我们将使用与 Nom 先生相同的方法:我们将为所有更新和呈现游戏世界的子屏幕以及子屏幕中的 UI 元素提供单独的更新和呈现方法。由于游戏屏幕代码有点长，我们将在这里把它分成多个清单。[清单 9-20](#list20) 显示了游戏屏幕的第一部分。

[**清单 9-20。**](#_list20)摘自 GameScreen.java；成员和构造函数
...
[**清单 9-21。**](#_list21)摘自 GameScreen.java；更新方法
...
在 updateRunning()方法中，我们首先检查用户是否触摸了右上角的暂停按钮。如果是这种情况，那么游戏就进入 GAME_PAUSED 状态。否则，我们用当前的增量时间和加速度计的 x 轴值来更新世界实例，它们负责水平移动 Bob。世界更新后，我们检查我们的分数字符串是否需要更新。我们还检查鲍勃是否已经到达城堡；如果有，我们进入 GAME_NEXT_LEVEL 状态，在[图 9-2](#Fig2) 的左上角屏幕显示消息，等待触摸事件生成下一关。如果游戏结束了，我们将分数字符串设置为 score: #score 或 new highscore: #score，这取决于所达到的分数是否是新的高分。然后，我们将分数添加到 Settings 类中，并告诉它将所有设置保存到 SD 卡中。此外，我们将游戏屏幕设置为 GAME_OVER 状态。
...
[**清单 9-22。**](#_list22)摘自 GameScreen.java；渲染方法
...
presentLevelEnd()方法呈现公主所在的字符串。。。屏幕上方还有另一座城堡里的绳子！在屏幕下方，如图[图 9-2](#Fig2) 所示。我们执行一些计算来使这些字符串水平居中。
...
[**清单 9-23。**GameScreen.java 其余地区的](#_list23)；pause()、resume()和 dispose()方法
...
我们只是确保当用户决定暂停应用程序时，我们的游戏屏幕是暂停的。
...
[**清单 9-24。**](#_list24)摘自 WorldRenderer.java；常数、成员和构造函数
...
[**清单 9-25。**WorldRenderer.java 其余地区的](#_list25)；实际的渲染代码
...
方法 renderItems() 呈现弹簧和硬币。对于弹簧，我们只使用我们在资产中定义的一个 TextureRegion，对于硬币，我们再次根据硬币的状态时间从动画中选择一个关键帧。
