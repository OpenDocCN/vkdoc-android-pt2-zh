...
Kotlin收藏馆实际上是 JDK 收藏馆的直接实例。不涉及包装的转换。因此，如果您在使用 Java 时没有忽略对集合的研究，那现在肯定会派上用场。尽管 Kotlin 没有定义自己的集合代码，但它确实为框架添加了相当多的便利函数，这是一个受欢迎的附加功能，因为它使集合更容易使用。
...
清单 [6-14](#PC18) 可以被称为过滤事物的“必要”方式。没什么不好——就是有点啰嗦，仅此而已。但是它可读性很好，即使对于刚开始编程的人也是如此。然而，在 Kotlin 中，更惯用的缩小集合的方法是使用*过滤器*函数。如果我们用过滤器来做这件事，它会像这样
