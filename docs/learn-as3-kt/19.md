...
*   **背景**T3】执行限制。[T0](http://bit.ly/bgexeclimit)。这是关于你的应用在后台运行时能做什么和不能做什么。不要以为因为不在 UI 线程里，就可以到处跑，想干嘛干嘛。这份文件谈到了这些限制；它还谈到了对广播接收器的限制。
...
我们需要添加一个继承自 BroadcastReceiver 的类。方法之一是从主菜单栏**文件** ➤ **新建** ➤ **Kotlin 文件/类**。或者，我们也可以使用项目工具窗口的 **app** ➤ **java** 文件夹中的上下文菜单，如图 [19-2](#Fig2) 所示。从那里，你可以去**新** ➤ **其他** ➤ **广播接收器**。
