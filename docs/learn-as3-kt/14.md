...
在 Android 的早期，当它只在手机上运行，没有任何高分辨率的屏幕时，activities 作为一种编写 UI 和与用户交互的方式就足够了。随后出现了平板电脑和高分辨率屏幕，创建可以在手机和平板电脑上良好运行的应用变得越来越困难。开发者面临着艰难的选择。要么选择功能最差的硬件作为目标，并使其成为最小公分母，要么通过根据设备的功能移除和添加 UI 元素来使应用适应一系列外形，这被证明是非常难以手动完成的。API 11(蜂巢)出来的时候，安卓用碎片解决了这个问题。
...
现在我们可以研究碎片了。让我们首先创建 *book_titles* 片段。创建一个新的布局资源文件，并将其命名为“book_titles”
