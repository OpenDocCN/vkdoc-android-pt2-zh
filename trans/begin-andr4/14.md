...
请注意，如果您的应用运行在较旧的 SDK 级别下，在 Honeycomb 和 Ice Cream Sandwich 发布之前，那么您的菜单将以老式的“按钮”样式出现，如图[Figure 14–9](#fig_14_9)所示。通过在您的`AndroidManifest.xml`中指定`android:targetSdkVersion`和`android:minSdkVersion`，您可以控制是使用旧行为还是新行为。第 29 章有一个有用的 SDK 版本列表。
...
**图 14–10。**T3【dynamic tab】应用，带单个初始标签
