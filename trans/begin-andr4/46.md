...
保持这个花名册不变将使您的应用程序可以使用 PhoneGap 为您的 JavaScript 提供的所有 API...和一个会吓跑很多用户的应用。毕竟，您的应用程序不太可能能够使用所有这些权限，更不用说证明这些权限的合理性了。
