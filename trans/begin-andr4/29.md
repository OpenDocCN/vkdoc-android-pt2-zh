...
然而，并不是动作栏的所有功能都是向后兼容的。在第 27 章的[中的`Menus/ActionBar`示例应用中，我们添加了一个自定义的`View`到动作栏，允许人们在不处理菜单和对话框的情况下添加单词到我们的列表中。然而，这需要一些只在 API 级别`11` (Android 3.0)和更高级别的代码。更高级的动作栏功能——超出了本书的范围——也有类似的需求。](27.html#ch27)
