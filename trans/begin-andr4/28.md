...
2011 年 Android 开发者面临的最大变化可能是 Android 3.0 引入了碎片系统，以及最近 Android 4.0 冰淇淋三明治将碎片系统合并到主代码库。片段是一个可选层，可以放在活动和小部件之间，旨在帮助您重新配置活动，以支持大屏幕(例如平板电脑)和小屏幕(例如手机)。然而，碎片系统也增加了额外的复杂性，这将需要 Android 开发者社区一些时间来适应。因此，使用片段的公共评论、博客帖子和示例应用就更少了，因为片段是在 Android 之后很久才被引入的。
