...
第 21 章中第一次提到的另一种方法是让服务发送一个广播`Intent`，这个广播可以被活动接收到...假设活动仍然存在并且没有暂停。该服务可以调用`sendBroadcast()`，提供一个`Intent`来识别广播，设计为由`BroadcastReceiver`接收。如果在清单中注册了`BroadcastReceiver`，这可以是特定于组件的广播(例如`new Intent(this, MyReceiver.class)`)。或者，它可以基于某个动作字符串，甚至可能是一个为第三方应用监听而记录和设计的字符串。
