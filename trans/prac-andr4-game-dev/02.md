...
现在您已经设置了您的设备和 avd，您终于可以运行`Hello World`应用程序了。在 Eclipse 中，您可以通过在 Package Explorer 视图中右键单击“hello world”项目，然后选择**Run As![images](img/U001.jpg)Android Application**(或者您可以单击工具栏上的 Run 按钮)来轻松实现这一点。然后，Eclipse 将在后台执行以下步骤:
...
如前一节所述，开始调试非常类似于运行应用程序。在 Package Explorer 视图中右键单击项目，并选择**Debug As![images](img/U001.jpg)Android Application。**这将为您的项目创建一个新的调试配置，就像简单地运行应用程序一样。您可以通过从上下文菜单中选择**调试为![images](img/U001.jpg)调试配置**来更改该调试配置的默认设置。
