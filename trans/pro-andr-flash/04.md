...
两个函数`touchMoveHandlerImage`和`touchTapHandlerImage`(顾名思义)处理 JPG 文件`fingersketch.jpg`的移动事件和单击事件，该文件存储在 Flex 应用程序的`images`子目录中。这两个函数包含一行调用相应函数`touchMove`和`touchTapHandler`的代码，这在本节前面已经讨论过了。
