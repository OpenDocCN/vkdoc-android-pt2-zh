...
对于您想要启用的每个权限，您可以复制`uses-permission`标记并用适当的权限名称替换`PERMISSION_NAME`占位符。[清单 5–3](#list_5_3)以适当的格式显示了所有可用的 Android 权限的例子，直接包含在应用程序描述符中。
