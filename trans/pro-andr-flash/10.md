...
应用的大小也是一个重要的考虑因素，因为它会影响应用从 Android Market 的初始下载及其启动性能。编译后的 ActionScript 实际上非常紧凑，因此静态资产(如嵌入到项目中的图像和视频)通常会决定应用的大小。
...
如果您正在开发一个基于 Flex 的应用，那么您将需要特别注意 UIComponents、GraphicElements 和 FXG 的使用。[Table 10–2](#tab_10_2)列出了使用这些不同对象类型的利弊。
