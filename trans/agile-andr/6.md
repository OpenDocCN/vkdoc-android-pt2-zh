...
TDD 意味着首先编写测试——这会失败——让测试通过，然后重构。我们的第一个功能是显示每个星座。使用 Android 向导创建一个名为占星的 Android 应用，其中包含一个空活动。我们的第一个测试使用我们在第 3 章中介绍的 Robolectric 来测试我们显示了 12 个标志(参见清单 [6-1](#FPar2) )。
...
该应用现在显示`ZodiacDetailActivity`的星座(见图 [6-7](#Fig7) )。
